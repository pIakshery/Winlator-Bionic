...
public class Container {
    ...
    public static final String DEFAULT_ENV_VARS = "ZINK_DESCRIPTORS=lazy ZINK_DEBUG=compact MESA_SHADER_CACHE_DISABLE=false MESA_SHADER_CACHE_MAX_SIZE=512MB mesa_glthread=true WINEESYNC=1 WINENTSYNC=0 TU_DEBUG=noconform,sysmem MANGOHUD=1 MANGOHUD_CONFIG=preset=3";
        ...
        for (Iterator<String> it = data.keys(); it.hasNext(); ) {
            ...
            switch (key) {
                ...
                case "rendererDriverId":
                    rendererDriverId = data.getString(key); break;
                case "rendererFilterMode" :
                    rendererFilterMode = data.getInt(key);
                    break;
                case "rendererRefreshRateLimit":
                    rendererRefreshRateLimit = data.getInt(key);
                    break;
                case "rendererSwapRB":
